...
function LinearProgress(props) {
  ...
  if (variant === 'determinate' || variant === 'buffer') {
    if (value !== undefined) {
      ...
      inlineStyles.bar1.WebkitTransform = inlineStyles.bar1.transform = `scaleX(${value / 100})`;
    } else {
      warning(
        false,
  ...
  if (variant === 'buffer') {
    if (valueBuffer !== undefined) {
      inlineStyles.bar2.WebkitTransform = inlineStyles.bar2.transform =
  ...
  return <div className={
    ...
    role="progressbar" {...rootProps
    } {...other
    } > {
      variant === 'buffer' ? < div className={
        dashedClass
      }
      /> : null} <
      div className={
